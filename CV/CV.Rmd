...
n_citations <- ggplot(data=history, aes(x=year, y=cites, fill=factor(ifelse(year=="2021","Highlighted","Normal")))) +
  geom_bar(stat="identity", fill = "#3f51b5") +
  labs(title = "", subtitle = "Number of citations per year") +
  scale_x_discrete(breaks = history$year, labels = history$year, limits = history$year) +
...
n_articles <- ggplot(pubs, aes(year, N)) + 
  ...
  scale_x_discrete(breaks = pubs$year, labels = pubs$year, limits = pubs$year) +
